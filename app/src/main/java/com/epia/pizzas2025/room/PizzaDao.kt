...
interface PizzaDao {
    ...
    // Método para obtener pizzas filtradas por tipo y ordenadas por descripción
    @Query("SELECT * FROM Pizza WHERE type = :type ORDER BY description ASC")
    fun getPizzasByTypeAndSort(type: String): MutableList<Pizza>

    @Query("SELECT * FROM Pizza ORDER BY description ASC")
    fun getAllPizzasSorted(): MutableList<Pizza>
